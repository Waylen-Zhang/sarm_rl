    ...
    def train(self):
        ...
        def train_step(batch, anno_type):
            ...
            with torch.no_grad():
                ...
                # cprint(f"img_emb shape: {img_emb.shape, img_emb}, lang_emb shape: {lang_emb.shape, lang_emb}", "cyan")
            ...
            if torch.rand(1).item() < 0.5:
                ...
                subtask_pred = subtask_model(img_emb, lang_emb, state, lens, stage_emb)
            if not ((gt_stage >= 0) & (gt_stage < num_classes)).all():
                cprint("Invalid gt_stage detected!","red")
                print("min:", gt_stage.min().item(),
                    "max:", gt_stage.max().item(),
