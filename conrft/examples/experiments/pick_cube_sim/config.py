...
from examples.experiments.pick_cube_sim.sarm_wrapper import SimStageAwareRewardWrapper
import uuid
...
#     reward_neg = -0.01
#     task_desc = "Pick up the red cube and lift it 10cm."
...
class TrainConfig(DefaultTrainingConfig):
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False, render_mode="human", stack_obs_num=1):
        ...
        if not fake_env:
            unique_node_name = f"gym_stage_client_{os.getpid()}_{uuid.uuid4().hex[:6]}"
            
            env = SimStageAwareRewardWrapper(
                env, 
                stage_buffer_size=5
                )
