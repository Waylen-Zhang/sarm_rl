...
def add_embeddings_to_trajectory(trajectory, model, tasks):
    ...
    for i in range(len(trajectory)):
        ...
        image_primary = observation["wrist_1"]
        image_wrist = observation["wrist_2"]
        # print(image_primary.shape)
        # print(image_wrist.shape)
        image_primary = np.stack([
            cv.resize(img, (256,256)) for img in image_primary
        ], axis=0)
        image_wrist = np.stack([
            cv.resize(img, (128,128)) for img in image_wrist
        ], axis=0)
        # Add batch dimension
